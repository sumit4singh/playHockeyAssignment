...
import { DocusignRequest } from './submit-docusign-request.interface'
import { HttpHeaders } from '@angular/common/http';

@Component({
  selector: 'app-post-env-from-docusign',
  templateUrl: './post-env-from-docusign.component.html',
  styleUrls: ['./post-env-from-docusign.component.scss']
})
export class PostEnvFromDocusignComponent {
  ...
  sendEnvelope() {
    ...
    this.requestBody = this.assignRequestBodyValue();
    this.requestHeader = this.assignRequestHeader();
    this.makePostCall();
  }

  makePostCall() {
    const url = "https://demo.docusign.net/restapi/v2/accounts/4094653/envelopes"
    this.httpHandler.post<any>(url, this.requestBody, this.requestHeader).subscribe({
      next: (response) => {
  ...
  assignRequestBodyValue(): DocusignRequest {
     const body: DocusignRequest = {
      recipients: {
        signers: [
          {
            email: this.email,
  ...
  assignRequestHeader(): any {
    return new HttpHeaders({
      'X-DocuSign-Authentication': '{"Username":"sumit.ipm03@gmail.com","Password":"Trippy@0320","IntegratorKey":"32590b5f-8a9c-496b-b16a-a7c52700d23d" }',
